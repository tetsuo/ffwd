...
static void
pair_map_insert(pair_merge_entry_t *map, int cap, uint64_t key, int rank, int merged_id) {
    int mask = cap - 1;
...
encode_pretokenized(const qwen_tokenizer_t *tok, const char *text, int **out_ids, int *out_n) {
    ...
    while (p < end) {
        ...
        if (append_encoded_piece(tok, (const char *)piece, (size_t)(p - piece), &ids, &n_ids,
                                 &cap) != 0) {
            free(ids);
            return -1;
        }
...
static int encode_bpe_word_into(const qwen_tokenizer_t *tok,
    ...
    for (;;) {
        ...
        rnk[best] = nxt[best] != -1 ? pair_merge_lookup(tok, ids[best], ids[nxt[best]], &mrg[best])
                                    : INT_MAX;
    }
