...
int main(void) {
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/ffwd-qwen3-test-XXXXXX",
    ...
    float embedding[4];
    float pooled[4];
    if (ffwd_model_forward_into(model, ws, ids, 4, states) != 0 ||
        ffwd_model_encode_into(model, ws, ids, 4, embedding) != 0 ||
        ffwd_pool_batch(cfg, states, (int[]){4}, 1, pooled) != 0) {
    ...
    int short_ids[] = {4, 15};
    ffwd_input_t inputs[] = {{ids, 4}, {short_ids, 2}};
    float batch[8];
    float single[4];
