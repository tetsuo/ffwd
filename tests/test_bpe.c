...
static void check_ids(const tok_bpe_t *tok,
                      ...
                      const char *label) {
    int n = 0;
    int *ids = tok_bpe_encode_with_workspace((tok_bpe_t *)tok, ws, text, &n);
...
static void check_roundtrip(const tok_bpe_t *tok,
                            tok_bpe_workspace_t *ws,
                            const char *text,
                            const char *want,
                            const char *label) {
    ...
    size_t cap = 1;
    for (int i = 0; i < n; i++)
        cap += strlen(tok_bpe_decode(tok, ids[i]));
    ...
    got[0] = '\0';
    for (int i = 0; i < n; i++)
        strcat(got, tok_bpe_decode(tok, ids[i]));
...
static void check_same_ids(
    const tok_bpe_t *tok, tok_bpe_workspace_t *ws, const char *a, const char *b, const char *label) {
    int na = 0, nb = 0;
    int *ia = tok_bpe_encode_with_workspace((tok_bpe_t *)tok, ws, a, &na);
    if (!ia) {
        TEST_ASSERT(0);
        return;
    }
    int *ib = tok_bpe_encode_with_workspace((tok_bpe_t *)tok, ws, b, &nb);
    if (!ib) {
        free(ia);
        TEST_ASSERT(0);
        return;
...
static void run_cases(const tok_bpe_t *tok, tok_bpe_workspace_t *ws, const char *label) {
...
static void test_loader_edges(const char *root) {
    ...
    TEST_ASSERT(tok_bpe_load("/nonexistent/vocab.json") == NULL);
    ...
    TEST_ASSERT(tok_bpe_load(path) == NULL);
    ...
    snprintf(path, sizeof(path), "%s/vocab.json", dir);
    tok_bpe_t *nom = tok_bpe_load(path);
    TEST_ASSERT(nom != NULL);
    if (nom) {
        tok_bpe_workspace_t *nws = tok_bpe_workspace_new();
        int n = 0;
        int *ids = tok_bpe_encode_with_workspace(nom, nws, "ab", &n);
        TEST_ASSERT(ids && n == 2 && ids[0] == 0 && ids[1] == 1);
        free(ids);
        tok_bpe_workspace_free(nws);
        tok_bpe_free(nom);
    }
    ...
    snprintf(path, sizeof(path), "%s/vocab.json", dir);
    tok_bpe_t *emp = tok_bpe_load(path);
    ...
    write_file(dir, "merges.txt", "#version: 0.2\nmalformed-line\na b\n");
    snprintf(path, sizeof(path), "%s/vocab.json", dir);
    tok_bpe_t *tok = tok_bpe_load(path);
    TEST_ASSERT(tok != NULL);
    if (tok) {
        TEST_ASSERT(tok_bpe_token_id(tok, "\xE2\x82\xAC") == 3);
        TEST_ASSERT(tok_bpe_token_id(tok, "x\ny") == 4);
        TEST_ASSERT(tok_bpe_token_id(tok, "q\"r") == 5);
        TEST_ASSERT(tok_bpe_token_id(tok, "s\\t") == 6);
        TEST_ASSERT(tok_bpe_token_id(tok, "p/q") == 7);
        TEST_ASSERT(tok_bpe_token_id(tok, "u\tv") == 8);
        TEST_ASSERT(tok_bpe_token_id(tok, "AZ") == 9);
        TEST_ASSERT(strcmp(tok_bpe_decode(tok, 2), "ab") == 0);
        /* Keys outside the GPT-2 byte alphabet decode to replacement. */
        TEST_ASSERT(strcmp(tok_bpe_decode(tok, 3), "?") == 0);
        tok_bpe_free(tok);
    }
    ...
    if (getcwd(saved_cwd, sizeof(saved_cwd)) && chdir(dir) == 0) {
        tok_bpe_t *rel = tok_bpe_load("vocab.json");
...
int main(void) {
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/ffwd-tok-test-XXXXXX",
    ...
    TEST_ASSERT(strcmp(tok_bpe_decode(tok, 'a'), "a") == 0);
    TEST_ASSERT(strcmp(tok_bpe_decode(tok, 259), "hello") == 0);
    TEST_ASSERT(strcmp(tok_bpe_decode(tok, -1), "") == 0);
    TEST_ASSERT(strcmp(tok_bpe_decode(tok, TF_VOCAB_SIZE), "") == 0);
    TEST_ASSERT(strcmp(tok_bpe_decode(NULL, 0), "") == 0);

    /* token_id lookups. */
    TEST_ASSERT(tok_bpe_token_id(tok, "hello") == 259);
    TEST_ASSERT(tok_bpe_token_id(tok, "<|endoftext|>") == TF_EOT_ID);
    TEST_ASSERT(tok_bpe_token_id(tok, "no-such-token") == -1);
    TEST_ASSERT(tok_bpe_token_id(NULL, "a") == -1);
    TEST_ASSERT(tok_bpe_token_id(tok, NULL) == -1);
     ...
     * zero capacity, empty text, and argument guards. */
    {
        int out[8];
        int n = -1;
        TEST_ASSERT(tok_bpe_encode_into(tok, ws, "hello world", out, 8, &n) == 0 && n == 3);
        ...
        n = -1;
        TEST_ASSERT(tok_bpe_encode_into(tok, ws, "hello world", out, 2, &n) == -2 && n == 3);
        TEST_ASSERT(out[0] == 259 && out[1] == 263);
        n = -1;
        TEST_ASSERT(tok_bpe_encode_into(tok, ws, "hello world", out, 0, &n) == -2 && n == 3);
        n = -1;
        TEST_ASSERT(tok_bpe_encode_into(tok, ws, "", out, 8, &n) == 0 && n == 0);
        TEST_ASSERT(tok_bpe_encode_into(tok, ws, NULL, out, 8, &n) == -1);
        TEST_ASSERT(tok_bpe_encode_into(NULL, ws, "x", out, 8, &n) == -1);
    }
    TEST_ASSERT(tok_bpe_encode(tok, "", NULL) == NULL);
    TEST_ASSERT(tok_bpe_encode(NULL, "x", NULL) == NULL);
    tok_bpe_workspace_free(NULL);
    tok_bpe_free(NULL);
