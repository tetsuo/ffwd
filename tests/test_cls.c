...
int main(void) {
    ...
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/embed-cls-test-XXXXXX", tmp);
    ...
    if (cfg->pooling_mode != EMBED_POOL_CLS ||
        cfg->attention_mode != EMBED_ATTENTION_BIDIRECTIONAL || !cfg->normalize_embeddings) {
    ...
    if (embed_l2_normalize(last, 4) != 0 || max_abs_diff(expected, last, 4) < 1e-4f) {
