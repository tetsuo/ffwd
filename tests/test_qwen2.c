...
int main(void) {
    ...
    snprintf(dir, sizeof(dir), "%s/embed-qwen2-test-XXXXXX", tmp);
    snprintf(zero_dir, sizeof(zero_dir), "%s/embed-qwen2-zero-bias-test-XXXXXX", tmp);
    ...
    embed_model_t *zero_model = embed_model_load(zero_dir);
    embed_workspace_t *ws = embed_workspace_new(model);
    embed_workspace_t *zero_ws = embed_workspace_new(zero_model);
    const embed_config_t *cfg = embed_model_config(model);
    if (!model || !zero_model || !ws || !zero_ws || !cfg) {
        fprintf(stderr, "model setup failed\n");
        return 1;
    }
    if (cfg->qk_norm || !cfg->qkv_bias || cfg->attention_mode != EMBED_ATTENTION_BIDIRECTIONAL ||
    ...
    if (embed_model_forward_into(model, ws, ids, 3, states) != 0 ||
        embed_model_encode_into(model, ws, ids, 3, embedding) != 0 ||
        embed_model_encode_into(zero_model, zero_ws, ids, 3, zero_embedding) != 0 ||
        embed_pool_batch(cfg, states, (int[]){3}, 1, pooled) != 0) {
    ...
    if (embed_model_forward_into(model, ws, changed_ids, 3, changed_states) != 0 ||
    ...
    int short_ids[] = {4, 15};
    embed_input_t inputs[] = {{ids, 3}, {short_ids, 2}};
    ...
    embed_workspace_free(zero_ws);
    embed_workspace_free(ws);
    embed_model_free(zero_model);
