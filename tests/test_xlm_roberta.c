...
static int write_fixture(const char *dir) {
    ...
    return tm_write_safetensors_with_prefix(path, "F32", specs,
                                            (int)(sizeof(specs) / sizeof(specs[0])), "roberta.");
}

int main(void) {
    const char *tmp = getenv("TMPDIR") ? getenv("TMPDIR") : "/tmp";
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/ffwd-xlm-roberta-test-XXXXXX", tmp);
    ...
    if (cfg->family != FFWD_FAMILY_BERT || cfg->position_id_offset != 2 ||
        cfg->type_vocab_size != 1 || cfg->pooling_mode != FFWD_POOL_MEAN ||
    ...
    if (ffwd_model_encode_into(model, ws, ids_ok, 6, out) != 0) {
        fprintf(stderr, "XLM-R forward failed at max valid position\n");
        return 1;
    }
    int ids_too_long[] = {0, 4, 5, 6, 7, 8, 2};
    if (ffwd_model_encode_into(model, ws, ids_too_long, 7, out) == 0) {
