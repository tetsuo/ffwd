 ...
 *             bench_sink += ffwd_dot_f32(x, x, 1024);
 *         free(x);
 *     }
 *
...
static void bench_begin(bench_state_t *b) { b->t0_ns = bench_now_ns(); }
...
static int bench_main(const bench_opts_t *o, const bench_case_t *cases, int n_cases) {
    FILE *jf = NULL;
    if (o->json_path) {
        ...
        fprintf(jf,
                "{\"host\":\"%s\",\"benchtime_ms\":%ld,\"count\":%d,"
                "\"meta\":\"%s\",\"benchmarks\":[",
    ...
    for (int c = 0; c < n_cases; c++) {
        ...
        while (ns_per_op * (double)n < target_ns) {
            ...
            if (next > n * 100)
                next = n * 100; /* distrust tiny timings */
            if (next <= n)
                next = n + 1;
            n = next;
    ...
    for (int s = 0; s < o->count; s++) {
        for (int c = 0; c < n_cases; c++)
            if (iters[c])
                all[(size_t)c * o->count + s] = bench_run_once(cases[c].fn, iters[c]);
    ...
    for (int c = 0; c < n_cases; c++) {
        if (!iters[c])
            continue;
        memcpy(samples, all + (size_t)c * o->count, (size_t)o->count * sizeof(double));

        if (jf) {
            fprintf(jf, "%s{\"name\":\"%s\",\"iters\":%ld,\"ns_per_op\":[", emitted ? "," : "",
                    cases[c].name, iters[c]);
            for (int s = 0; s < o->count; s++)
        ...
        double spread = o->count > 1 ? 100.0 * (samples[o->count - 1] - samples[0]) / med : 0.0;
        printf("%-44s %10ld iters %14.1f ns/op  (spread %.1f%%)\n", cases[c].name, iters[c], med,
               spread);
        fflush(stdout);
        emitted++;
    }
