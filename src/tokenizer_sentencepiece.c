/*
 * tokenizer_sentencepiece.c - SentencePiece Unigram tokenizer.
 *
...
struct sentencepiece_workspace {
    char *norm;
    size_t norm_len;
    size_t norm_cap;
    sp_best_t *best;
...
static int reserve_best(sentencepiece_workspace_t *ws, int need) {
    if (need <= ws->best_cap)
        return 0;
...
static int reserve_ids(sentencepiece_workspace_t *ws, int need) {
    if (need <= ws->ids_cap)
        return 0;
...
static int
parse_piece(sentencepiece_tokenizer_t *tok, int idx, const unsigned char *data, size_t len) {
...
static int parse_trainer(sentencepiece_tokenizer_t *tok, const unsigned char *data, size_t len) {
    const unsigned char *p = data, *end = data + len;
...
static int parse_normalizer(sentencepiece_tokenizer_t *tok, const unsigned char *data, size_t len) {
    const unsigned char *p = data, *end = data + len;
    while (p < end) {
        int field = 0, wire = 0;
        if (pb_read_key(&p, end, &field, &wire) != 0)
...
static int parse_model(sentencepiece_tokenizer_t *tok, const unsigned char *data, size_t len) {
    ...
    while (p < end) {
        ...
        if (wire == 2 && field == 1) {
            ...
            size_t msg_len = 0;
            if (pb_read_len(&p, end, &msg, &msg_len) != 0 ||
                parse_normalizer(tok, msg, msg_len) != 0)
                return -1;
        } else if (pb_skip(&p, end, wire) != 0) {
            return -1;
        }
...
static int raw_piece_id(const sentencepiece_tokenizer_t *tok, const char *piece) {
    if (!tok || !piece)
        return -1;
    return map_get((const sp_entry_t *)tok->piece_map, tok->piece_map_cap, piece,
                   (int)strlen(piece));
}

static int raw_to_model_id(const sentencepiece_tokenizer_t *tok, int raw_id) {
    if (!tok || raw_id < 0 || raw_id >= tok->piece_count)
        return -1;
    if (tok->id_map == SENTENCEPIECE_XLM_ROBERTA) {
        if (raw_id == 1)
            return 0; /* <s> */
        if (raw_id == 2)
            return 2; /* </s> */
...
static int model_to_raw_id(const sentencepiece_tokenizer_t *tok, int id) {
    if (!tok || id < 0)
        return -1;
    if (tok->id_map == SENTENCEPIECE_XLM_ROBERTA) {
        if (id == 0)
...
static uint32_t xcda_base(const sentencepiece_tokenizer_t *tok, size_t idx) {
    uint32_t v = tok->xcda[idx];
    return (v >> 10) << ((v & (1u << 9)) >> 6);
}

static uint32_t xcda_lcheck(const sentencepiece_tokenizer_t *tok, size_t idx) {
    return tok->xcda[idx] & ((1u << 31) | 0xffu);
}

static int xcda_leaf(const sentencepiece_tokenizer_t *tok, size_t idx) {
    return (tok->xcda[idx] >> 8) & 1u;
}

static uint32_t xcda_value(const sentencepiece_tokenizer_t *tok, size_t idx) {
...
static norm_prefix_t normalize_prefix(const sentencepiece_tokenizer_t *tok,
                                      const char *input,
                                      size_t input_len,
                                      size_t off) {
...
static int normalize_into(const sentencepiece_tokenizer_t *tok,
            ...
            if (c != ' ') {
                if (!processing_non_ws) {
                    ...
                    if ((tok->add_dummy_prefix && !space_prepended) ||
                        ...
                        if (append_bytes(&ws->norm, &ws->norm_len, &ws->norm_cap, space_s,
                                         space_n) != 0)
                            return -1;
                        space_prepended = 1;
                    }
...
static double compute_unknown_score(const sentencepiece_tokenizer_t *tok) {
...
static int segment(sentencepiece_tokenizer_t const *tok, sentencepiece_workspace_t *ws) {
    ...
    for (size_t off = 0; off < n;) {
        ...
        for (size_t end = off + cp_len; end <= n && end - off <= (size_t)tok->max_piece_len;) {
            int raw = map_get((const sp_entry_t *)tok->piece_map, tok->piece_map_cap,
                              ws->norm + off, (int)(end - off));
...
sentencepiece_tokenizer_t *sentencepiece_tokenizer_load(const char *model_dir) {
    if (!model_dir)
        return NULL;
    char path[1024];
    ...
    sentencepiece_tokenizer_t *tok =
        (sentencepiece_tokenizer_t *)calloc(1, sizeof(sentencepiece_tokenizer_t));
    if (!tok) {
        free(data);
        return NULL;
    }
    ...
    tok->id_map =
        detect_xlm_roberta(model_dir) ? SENTENCEPIECE_XLM_ROBERTA : SENTENCEPIECE_IDENTITY;
    ...
    tok->pad_id = tok->id_map == SENTENCEPIECE_XLM_ROBERTA
                      ? 1
                      : raw_to_model_id(tok, raw_piece_id(tok, "<pad>"));
    tok->mask_id = tok->id_map == SENTENCEPIECE_XLM_ROBERTA
                       ? tok->piece_count + 1
                       : raw_to_model_id(tok, raw_piece_id(tok, "<mask>"));
    tok->vocab_size =
        tok->id_map == SENTENCEPIECE_XLM_ROBERTA ? tok->piece_count + 2 : tok->piece_count;
...
const char *sentencepiece_tokenizer_decode(const sentencepiece_tokenizer_t *tok, int id) {
    if (!tok || id < 0)
        return NULL;
    if (tok->id_map == SENTENCEPIECE_XLM_ROBERTA) {
        if (id == 1)
...
int sentencepiece_tokenizer_token_id(const sentencepiece_tokenizer_t *tok, const char *token) {
    if (!tok || !token)
        return -1;
    if (tok->id_map == SENTENCEPIECE_XLM_ROBERTA) {
        if (!strcmp(token, "<s>"))
...
int sentencepiece_tokenizer_encode_into(const sentencepiece_tokenizer_t *tok,
                                        sentencepiece_workspace_t *ws,
                                        const char *text,
                                        int *out_ids,
                                        int out_cap,
...
int *sentencepiece_tokenizer_encode_with_workspace(const sentencepiece_tokenizer_t *tok,
                                                   sentencepiece_workspace_t *ws,
                                                   const char *text,
                                                   int *out_n_tokens) {
    if (out_n_tokens)
    ...
    if (!tok || !ws || !text)
        return NULL;
    int n = 0;
    int rc = sentencepiece_tokenizer_encode_into(tok, ws, text, NULL, 0, &n);
    if (rc < 0 && rc != -2)
        return NULL;
    if (out_n_tokens)
        *out_n_tokens = n;
...
int *sentencepiece_tokenizer_encode(const sentencepiece_tokenizer_t *tok,
                                    const char *text,
                                    int *out_n_tokens) {
    ...
    int *ids = sentencepiece_tokenizer_encode_with_workspace(tok, ws, text, out_n_tokens);
    sentencepiece_workspace_free(ws);
    return ids;
}

sentencepiece_workspace_t *sentencepiece_workspace_new(void) {
    return (sentencepiece_workspace_t *)calloc(1, sizeof(sentencepiece_workspace_t));
}

void sentencepiece_workspace_free(sentencepiece_workspace_t *ws) {
    if (!ws)
        return;
    free(ws->norm);
    free(ws->best);
...
void sentencepiece_tokenizer_free(sentencepiece_tokenizer_t *tok) {
    if (!tok)
        return;
    if (tok->id_to_piece) {
        for (int i = 0; i < tok->piece_count; i++)
