 ...
 * inputs into a *_request; the worker (server_schedule.c) groups compatible
 * ready requests and calls the matching execute_*_request_list, which runs the
 * batched forward pass through model_embed_* and renders each response via
 * server_encode.c. Rerank runs MaxSim over late-interaction token vectors.
 * ======================================================================== */
