...
/* The global verbose flag (defined in model.c); used only for optional pool
 * diagnostics below. Declared here to keep the thread pool independent of the
 * kernel/model headers. */
extern int embed_verbose;
...
int embed_num_threads(void) { return tp.n_threads; }
