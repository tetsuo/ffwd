...
struct ffwd_mlx_late_vectors {
    ffwd_mlx_late_ctx_t *owner;
...
static mlx_dtype mlx_weight_dtype(const ffwd_mlx_ctx_t *ctx) { return ctx->weight_dtype; }
...
static mlx_array load_tensor(multi_safetensors_t *ms, const char *name, const int *shape, int ndim) {
...
static mlx_array linear(mlx_array x, const mlx_linear_t *W, mlx_stream s) {
    if (W->quantized) {
        ...
        mlx_array null_arr = (mlx_array){0};
        mlx_quantized_matmul(&res, x, W->w, W->scales, arr_ok(W->biases) ? W->biases : null_arr, true,
                             gs, bits, "affine", s);
        return res;
    }
    mlx_array Wt;
...
static ffwd_mlx_options_t normalize_mlx_options(const ffwd_mlx_options_t *opts) {
    ffwd_mlx_options_t out = {0};
    if (opts)
        out = *opts;
    if (out.quantize_bits != 0 && out.quantize_bits != 8) {
        fprintf(stderr, "mlx: --gpu-quant-bits supports only 8\n");
        out.quantize_bits = -1;
    }
...
static int load_mlx_bert_weights(ffwd_mlx_ctx_t *ctx, int layer_start, int layer_end) {
    ...
    if (layer_start == 0) {
        ...
        ctx->ffwd_ln_w = load_tensor(ctx->ms, name, ln_shape, 1);
...
static ffwd_mlx_ctx_t *mlx_load_range_ex(const char *model_dir,
    ...
    ffwd_mlx_ctx_t *ctx = calloc(1, sizeof(ffwd_mlx_ctx_t));
    if (!ctx)
        return NULL;
    ctx->quantize_bits = options.quantize_bits;
    ctx->quantize_group_size = options.quantize_group_size;

    if (ffwd_config_parse(&ctx->config, model_dir) != 0) {
        free(ctx);
        return NULL;
    }
...
loaded:
    ...
    if (ctx->weight_dtype == MLX_BFLOAT16 && !ctx->quantize_bits) {
        ...
        upcast_f32_inplace(&ctx->ffwd_ln_w, ctx->stream);
        upcast_f32_inplace(&ctx->ffwd_ln_b, ctx->stream);
    }
...
ffwd_mlx_ctx_t *ffwd_mlx_load(const char *model_dir) {
...
void ffwd_mlx_free(ffwd_mlx_ctx_t *ctx) {
    if (!ctx)
        return;
    ...
    mlx_array_free(ctx->ffwd_ln_w);
    mlx_array_free(ctx->ffwd_ln_b);
    if (ctx->stream.ctx)
...
ffwd_mlx_late_ctx_t *ffwd_mlx_late_load_with_options(const char *model_dir,
    ...
    ffwd_mlx_late_ctx_t *late = calloc(1, sizeof(*late));
    if (!late)
        return NULL;
...
fail:
    ffwd_mlx_late_free(late);
    return NULL;
}

ffwd_mlx_late_ctx_t *ffwd_mlx_late_load(const char *model_dir) {
    return ffwd_mlx_late_load_with_options(model_dir, NULL);
}

void ffwd_mlx_late_free(ffwd_mlx_late_ctx_t *ctx) {
    if (!ctx)
        return;
    free_linear(&ctx->projection);
    if (ctx->dense_ms)
        multi_safetensors_close(ctx->dense_ms);
    ffwd_mlx_free(ctx->base);
    free(ctx);
}

const ffwd_config_t *ffwd_mlx_late_config(const ffwd_mlx_late_ctx_t *ctx) {
...
static mlx_array mlx_forward_bert(ffwd_mlx_ctx_t *ctx,
                                  mlx_array x,
                                  int batch,
                                  int max_seq,
                                  int has_padding,
...
static mlx_array mlx_forward_layers(ffwd_mlx_ctx_t *ctx,
                                    mlx_array x,
                                    int batch,
                                    int max_seq,
                                    int has_padding,
    ...
    float scale = 1.0f / sqrtf((float)head_dim);
    mlx_array null_arr = (mlx_array){0};
    const char *mask_mode = c->attention_mode == FFWD_ATTENTION_CAUSAL ? "causal" : "";
    ...
    int free_mask = 0;
    int mixed_bf16 = mlx_weight_dtype(ctx) == MLX_BFLOAT16 && !ctx->layers[layer_start].wq.quantized;
    if (mixed_bf16) {
...
ffwd_mlx_late_vectors_t *ffwd_mlx_late_encode_tokens_device(ffwd_mlx_late_ctx_t *ctx,
    ...
    ffwd_mlx_late_vectors_t *result = NULL;
    ...
    result = (ffwd_mlx_late_vectors_t *)calloc(1, sizeof(*result));
    if (!result)
        goto cleanup;
    result->owner = ctx;
...
void ffwd_mlx_late_vectors_free(ffwd_mlx_late_vectors_t *vecs) {
    if (!vecs)
        return;
...
int ffwd_mlx_late_vectors_token_count(const ffwd_mlx_late_vectors_t *vecs) {
 ...
 * ffwd_mlx_late_maxsim_batch_device consumes. This collapses N per-candidate
...
ffwd_mlx_late_vectors_t *ffwd_mlx_late_vectors_concat(
    ...
    for (int i = 0; i < count; i++) {
        const ffwd_mlx_late_vectors_t *v = items[i];
    ...
    ffwd_mlx_late_vectors_t *out = (ffwd_mlx_late_vectors_t *)calloc(1, sizeof(*out));
    if (!out) {
        mlx_array_free(joined);
        return NULL;
    }
...
int ffwd_mlx_late_encode_tokens(ffwd_mlx_late_ctx_t *ctx,
    ...
    if (!vecs)
        return -1;
    int rc = ffwd_mlx_late_vectors_copy(vecs, out_vectors);
    ffwd_mlx_late_vectors_free(vecs);
    return rc;
}
...
static int ffwd_mlx_encode_batch_dense(ffwd_mlx_ctx_t *ctx,
    ...
    int has_padding = 0;
    int causal = c->attention_mode == FFWD_ATTENTION_CAUSAL;
    int cls_pool = c->pooling_mode == FFWD_POOL_CLS;
    ...
    int single_token_pool = c->pooling_mode == FFWD_POOL_LAST_TOKEN || cls_pool;
    ...
    mlx_array last_indices = single_token_pool
    ...
    if (c->family == FFWD_FAMILY_BERT) {
        ...
        mlx_fast_layer_norm(&xe, xt, ctx->ffwd_ln_w, ctx->ffwd_ln_b, c->layer_norm_eps, S);
        mlx_array_free(xt);
        x_normed =
    ...
    if (data) {
        ...
        if (c->normalize_embeddings) {
            rc = 0;
            for (int b = 0; b < batch; b++) {
                if (ffwd_l2_normalize(out_embeddings + (size_t)b * hidden, hidden) != 0) {
                    rc = -1;
                    break;
                }
            }
...
static int mlx_batch_should_split(const ffwd_input_t *inputs,
                                  int batch,
...
int ffwd_mlx_encode_batch(ffwd_mlx_ctx_t *ctx,
    ...
    if (!mlx_batch_should_split(inputs, batch, NULL, NULL))
        return ffwd_mlx_encode_batch_dense(ctx, inputs, batch, out_embeddings);

    const int hidden = ctx->config.hidden_size;
    mlx_batch_item_t *items = (mlx_batch_item_t *)malloc((size_t)batch * sizeof(*items));
    ffwd_input_t *group_inputs = (ffwd_input_t *)malloc((size_t)batch * sizeof(*group_inputs));
    ...
    for (int start = 0; start < batch && rc == 0;) {
        ...
        rc = ffwd_mlx_encode_batch_dense(ctx, group_inputs, group_count, group_out);
        if (rc != 0)
            break;
        for (int i = 0; i < group_count; i++) {
...
typedef struct {
    int index;
    int span_offset;
    const ffwd_context_input_t *input;
...
static int ffwd_mlx_encode_spans_batch_dense(ffwd_mlx_ctx_t *ctx,
    ...
    for (int b = 0; b < batch; b++) {
        const ffwd_input_t *input = &inputs[b].input;
        for (int t = 0; t < max_seq; t++) {
    ...
    if (mlx_weight_dtype(ctx) == MLX_BFLOAT16) {
        ...
        for (int b = 0; b < batch; b++) {
            for (int s = 0; s < inputs[b].n_spans; s++) {
                const ffwd_span_t *span = &inputs[b].spans[s];
...
int ffwd_mlx_encode_spans_batch(ffwd_mlx_ctx_t *ctx,
    ...
    if (!mlx_context_batch_should_split(inputs, batch, &total_spans))
        return ffwd_mlx_encode_spans_batch_dense(ctx, inputs, batch, out_embeddings);

    const int hidden = ctx->config.hidden_size;
    mlx_context_batch_item_t *items =
        (mlx_context_batch_item_t *)malloc((size_t)batch * sizeof(*items));
    ffwd_context_input_t *group_inputs =
        (ffwd_context_input_t *)malloc((size_t)batch * sizeof(*group_inputs));
    size_t group_values, group_bytes;
    ...
    for (int start = 0; start < batch && rc == 0;) {
        ...
        for (int i = 0; i < group_count; i++)
            group_inputs[i] = *items[start + i].input;
        rc = ffwd_mlx_encode_spans_batch_dense(ctx, group_inputs, group_count, group_out);
        if (rc != 0)
            break;
        int src_span = 0;
        for (int i = 0; i < group_count; i++) {
            ...
            memcpy(out_embeddings + (size_t)dst_span * hidden, group_out + (size_t)src_span * hidden,
...
int ffwd_mlx_encode_spans(ffwd_mlx_ctx_t *ctx,
    ...
    return ffwd_mlx_encode_spans_batch(ctx, &input, 1, out_embeddings);
}

float *ffwd_mlx_encode(ffwd_mlx_ctx_t *ctx, const int *token_ids, int n_tokens) {
    ...
    if (ffwd_mlx_encode_into(ctx, token_ids, n_tokens, out) != 0) {
        free(out);
        return NULL;
    }
    return out;
