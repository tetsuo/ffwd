...
static void bf16_row_to_f32(float *dst, const uint16_t *src, int n) {
    for (int i = 0; i < n; i++)
        dst[i] = ffwd_bf16_to_f32(src[i]);
}

static void copy_weight_row(float *dst, const ffwd_weight_ref_t *w, size_t row, int row_width) {
...
static int ensure_bf16_widen(ffwd_workspace_t *ws, size_t count) {
...
void linear_nobias_weight(ffwd_workspace_t *ws,
                          ...
                          int out_dim) {
    if (w->dtype == DTYPE_F32) {
        linear_nobias(y, x, (const float *)w->data, seq_len, in_dim, out_dim);
        return;
    }
    if (seq_len > 16) {
        ...
        if (ensure_bf16_widen(ws, count) == 0) {
            bf16_to_f32_buf(ws->bf16_widen, (const uint16_t *)w->data, count);
            linear_nobias(y, x, ws->bf16_widen, seq_len, in_dim, out_dim);
            return;
        }
    }
    linear_nobias_bf16(y, x, (const uint16_t *)w->data, seq_len, in_dim, out_dim);
}

static void linear_qkv_weight(ffwd_workspace_t *ws,
                              float *q,
                              float *k,
                              float *v,
                              const float *x,
                              const ffwd_weight_ref_t *wq,
                              const ffwd_weight_ref_t *wk,
                              const ffwd_weight_ref_t *wv,
                              int seq_len,
                              int in_dim,
                              int q_dim,
                              int kv_dim) {
    if (wq->dtype == DTYPE_BF16 && wk->dtype == DTYPE_BF16 && wv->dtype == DTYPE_BF16 &&
        seq_len <= FFWD_BF16_QKV_FUSE_MAX_SEQ) {
        linear_nobias_bf16_qkv(q, k, v, x, (const uint16_t *)wq->data, (const uint16_t *)wk->data,
                               (const uint16_t *)wv->data, seq_len, in_dim, q_dim, kv_dim);
        return;
    }
...
static void linear_pair_weight(ffwd_workspace_t *ws,
                               float *a,
                               float *b,
                               const float *x,
                               const ffwd_weight_ref_t *wa,
                               const ffwd_weight_ref_t *wb,
                               int seq_len,
                               int in_dim,
                               int a_dim,
                               int b_dim) {
    if (wa->dtype == DTYPE_BF16 && wb->dtype == DTYPE_BF16 &&
        seq_len <= FFWD_BF16_PAIR_FUSE_MAX_SEQ) {
        linear_nobias_bf16_pair(a, b, x, (const uint16_t *)wa->data, (const uint16_t *)wb->data,
...
static ffwd_attention_fn attention_for_config(const ffwd_config_t *cfg) {
    return cfg->attention_mode == FFWD_ATTENTION_CAUSAL
               ? causal_gqa_attention_packed_with_scratch
               : bidirectional_gqa_attention_packed_with_scratch;
}
...
static int forward_packed_slice_inplace(const ffwd_model_t *model,
                                        ffwd_workspace_t *ws,
    ...
    if (!model || !ws || ws->model != model || !inputs || batch <= 0 || !offsets || total_seq <= 0 ||
        max_seq <= 0)
        return -1;

    const ffwd_config_t *cfg = &model->config;
    const ffwd_weights_t *w = &model->weights;
    ...
    for (int layer = layer_start; layer < layer_end; layer++) {
        const ffwd_layer_t *l = &w->layers[layer];

        rms_norm(x_norm, x, l->input_norm, total_seq, hidden, eps);

        linear_qkv_weight(ws, q_buf, k_buf, v_buf, x_norm, &l->wq, &l->wk, &l->wv, total_seq, hidden,
                          q_dim, kv_dim);
        ...
        if (cfg->qk_norm) {
            rms_norm_per_head(q_buf, l->q_norm, total_seq, n_heads, head_dim, eps);
            rms_norm_per_head(k_buf, l->k_norm, total_seq, n_kv_heads, head_dim, eps);
        }

        for (int b = 0; b < batch; b++) {
            int start = offsets[b];
            int len = offsets[b + 1] - start;
            apply_rope_neox(q_buf + (size_t)start * q_dim, rope_cos, rope_sin, len, n_heads,
                            head_dim);
            apply_rope_neox(k_buf + (size_t)start * kv_dim, rope_cos, rope_sin, len, n_kv_heads,
                            head_dim);
        }

        ffwd_attention_fn attention = attention_for_config(cfg);
        attention(attn_out, q_buf, k_buf, v_buf, offsets, batch, n_heads, n_kv_heads, head_dim, scale,
                  ws->attn_scores, ws->attn_scores_bytes);

        linear_nobias_weight(ws, proj_out, attn_out, &l->wo, total_seq, q_dim, hidden);
        add_inplace(x, proj_out, total_seq * hidden);

        rms_norm(x_norm, x, l->post_attn_norm, total_seq, hidden, eps);

        linear_pair_weight(ws, ffn_gate, ffn_up, x_norm, &l->gate_proj, &l->up_proj, total_seq,
                           hidden, inter, inter);
        silu_mul_inplace(ffn_gate, ffn_up, total_seq * inter);
        linear_nobias_weight(ws, proj_out, ffn_gate, &l->down_proj, total_seq, inter, hidden);
        add_inplace(x, proj_out, total_seq * hidden);
    }

    if (apply_final_norm)
        rms_norm(x, x, w->norm, total_seq, hidden, eps);
    return 0;
}
...
static int forward_packed_bert(const ffwd_model_t *model,
    ...
    if (max_seq > cfg->max_position_embeddings - cfg->position_id_offset) {
        fprintf(stderr,
                "ffwd: sequence length %d plus position offset %d exceeds max_position_embeddings "
                "%d\n",
    ...
    for (int b = 0; b < batch; b++) {
        ...
        for (int i = 0; i < inputs[b].n_tokens; i++) {
            ...
            add_inplace(row, w->position_embeddings + (size_t)pos * hidden, hidden);
            add_inplace(row, w->token_type_embeddings, hidden);
        }
    }
    layer_norm(x, x, w->ffwd_ln_w, w->ffwd_ln_b, total_seq, hidden, eps);
    ...
    for (int layer = 0; layer < cfg->n_layers; layer++) {
        ...
        ffwd_attention_fn attention = attention_for_config(cfg);
        attention(attn_out, q_buf, k_buf, v_buf, offsets, batch, n_heads, n_heads, head_dim, scale,
                  ws->attn_scores, ws->attn_scores_bytes);

        linear_nobias_weight(ws, proj_out, attn_out, &l->wo, total_seq, hidden, hidden);
        add_bias_rows(proj_out, l->o_bias, total_seq, hidden);
        add_inplace(x, proj_out, total_seq * hidden);
        layer_norm(x, x, l->input_norm, l->attn_ln_bias, total_seq, hidden, eps);
        ...
        if (cfg->ffn_act == FFWD_ACT_GELU_TANH)
            gelu_tanh_inplace(ffn_up, total_seq * inter);
        else
            gelu_inplace(ffn_up, total_seq * inter);
        ...
        add_inplace(x, proj_out, total_seq * hidden);
        layer_norm(x, x, l->post_attn_norm, l->ffn_ln_bias, total_seq, hidden, eps);
    }
    return 0;
}

static int pool_embeddings(const ffwd_model_t *model,
                           const ffwd_workspace_t *ws,
                           const int *offsets,
                           int batch,
    ...
    for (int b = 0; b < batch; b++) {
        ...
        if (cfg->normalize_embeddings && ffwd_l2_normalize(emb, hidden) != 0)
            return -1;
    }
    return 0;
}
...
int ffwd_model_encode_spans_batch(const ffwd_model_t *model,
    ...
    if (ensure_offsets(ws, batch) != 0)
        return -1;
    if ((size_t)batch > SIZE_MAX / sizeof(ffwd_input_t))
        return -1;
    ffwd_input_t *packed_inputs = malloc((size_t)batch * sizeof(*packed_inputs));
    if (!packed_inputs)
        return -1;
...
int ffwd_model_encode_spans(const ffwd_model_t *model,
    ...
    return ffwd_model_encode_spans_batch(model, ws, &input, 1, out_embeddings);
}
...
float *ffwd_model_forward(const ffwd_model_t *model,
    ...
    if (ffwd_model_forward_into(model, ws, token_ids, n_tokens, out) != 0) {
        free(out);
        return NULL;
    }
    return out;
...
int ffwd_model_encode_batch(const ffwd_model_t *model,
    ...
    int rc = model->config.family == FFWD_FAMILY_BERT
...
int ffwd_pool_batch(const ffwd_config_t *cfg,
    ...
    for (int b = 0; b < batch; b++) {
        ...
        if (cfg->pooling_mode == FFWD_POOL_LAST_TOKEN || cfg->pooling_mode == FFWD_POOL_CLS) {
            ...
            int idx = cfg->pooling_mode == FFWD_POOL_CLS ? offset : offset + len - 1;
        ...
        if (cfg->normalize_embeddings && ffwd_l2_normalize(emb, cfg->hidden_size) != 0)
            return -1;
        offset += len;
    }
    return 0;
...
float *ffwd_model_encode(const ffwd_model_t *model,
    ...
    if (ffwd_model_encode_into(model, ws, token_ids, n_tokens, emb) != 0) {
        free(emb);
        return NULL;
    }
    return emb;
