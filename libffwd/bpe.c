...
static void init_gpt2_mapping(void) {
    ...
    for (int b = 0; b < 256; b++) {
        ...
        if (is_normal)
            gpt2_byte_to_unicode[b] = b;
        else
            gpt2_byte_to_unicode[b] = 256 + n++;
    }

    for (int b = 0; b < 256; b++) {
        int cp = gpt2_byte_to_unicode[b];
        if (cp < 512)
            gpt2_unicode_to_byte[cp] = b;
    }
...
static int map_get_pair(const str_int_entry_t *map, int cap, const char *a, const char *b) {
    if (!map || cap <= 0 || !a || !b)
        return -1;
    int mask = cap - 1;
    ...
    for (int i = 0; i < cap; i++) {
        ...
        if (!map[idx].key)
            return -1;
        if (pair_key_eq(map[idx].key, a, b))
            return map[idx].value;
    }
    return -1;
}
...
static void pair_map_insert(pair_merge_entry_t *map, int cap, uint64_t key, int rank, int merged_id) {
    int mask = cap - 1;
    ...
    for (int probe = 0; probe < cap; probe++, idx = (idx + 1) & mask) {
        ...
        if (map[idx].key == key)
            return; /* first (lowest) rank wins */
    }
}

/* Returns the rank and sets *merged_id, or INT_MAX when the pair never
 * merges. */
static int pair_merge_lookup(const tok_bpe_t *tok, int left, int right, int *merged_id) {
    ...
    int idx = (int)(pair_hash(key) & (uint64_t)mask);
    for (int probe = 0; probe < tok->int_merges_cap; probe++, idx = (idx + 1) & mask) {
        if (map[idx].key == UINT64_MAX)
            return INT_MAX;
        if (map[idx].key == key) {
...
static int merge_rank(const tok_bpe_t *tok, const char *a, const char *b) {
    if (!tok->merge_map || tok->merge_map_cap <= 0)
        return INT_MAX;

    int rank = map_get_pair((const str_int_entry_t *)tok->merge_map, tok->merge_map_cap, a, b);
...
static int encode_bpe_word(const tok_bpe_t *tok, const char *mapped, int **out_ids, int *out_n);
...
utf8_decode_cp_len(const unsigned char *p, const unsigned char *end, int *cp_out, int *len_out) {
    ...
    if ((c & 0xF0) == 0xE0 && p + 2 < end && (p[1] & 0xC0) == 0x80 && (p[2] & 0xC0) == 0x80) {
        *cp_out = ((c & 0x0F) << 12) | ((p[1] & 0x3F) << 6) | (p[2] & 0x3F);
        *len_out = 3;
        return 1;
    }
    if ((c & 0xF8) == 0xF0 && p + 3 < end && (p[1] & 0xC0) == 0x80 && (p[2] & 0xC0) == 0x80 &&
        (p[3] & 0xC0) == 0x80) {
        *cp_out = ((c & 0x07) << 18) | ((p[1] & 0x3F) << 12) | ((p[2] & 0x3F) << 6) | (p[3] & 0x3F);
        *len_out = 4;
        return 1;
    }
...
static const unsigned char ascii_cp_class[128] = {
    ...
    ['0' ... '9'] = CPC_NUMBER, ['A' ... 'Z'] = CPC_LETTER, ['a' ... 'z'] = CPC_LETTER,
};

static int is_number_cp(int cp) {
    if ((unsigned)cp < 0x80)
        return ascii_cp_class[cp] & CPC_NUMBER;
...
static int is_space_cp(int cp) {
    if ((unsigned)cp < 0x80)
        return ascii_cp_class[cp] & CPC_SPACE;
...
static int is_symbolish_cp(int cp) {
    ...
    if (cp >= 0x0300 && cp <= 0x036F)
        return 1;
    if (cp >= 0x00A1 && cp <= 0x00BF && cp != 0x00AA && cp != 0x00B5 && cp != 0x00BA)
        return 1;
...
static char *normalize_nfc_latin(const char *text) {
    ...
    while (p < end) {
        ...
        int cp = 0, len = 0;
        if (!utf8_decode_cp_len(p, end, &cp, &len))
            break;
        p += len;
...
static int is_letter_cp(int cp) {
    if ((unsigned)cp < 0x80)
        return ascii_cp_class[cp] & CPC_LETTER;
    if (is_space_cp(cp) || is_number_cp(cp) || is_symbolish_cp(cp))
        return 0;
    return 1;
}

static int is_word_cp(int cp) { return is_letter_cp(cp) || is_number_cp(cp); }

static int is_newline_cp(int cp) { return cp == '\n' || cp == '\r'; }
...
static int append_encoded_piece(
    const tok_bpe_t *tok, const char *piece, size_t piece_len, int **ids, int *n_ids, int *cap) {
    if (piece_len == 0)
        return 0;
...
static int encode_pretokenized(const tok_bpe_t *tok, const char *text, int **out_ids, int *out_n) {
    ...
    while (p < end) {
        ...
        if (clen > 0) {
            ...
            if (!is_newline_cp(cp) && !is_word_cp(cp) && has_next && is_letter_cp(next_cp)) {
                p = next + next_len;
                while (p < end) {
                    int c = 0, l = 0;
                ...
                if (saw_newline) {
                    p = q;
                } else if (cp == ' ' && has_next && !is_space_cp(next_cp) && !is_word_cp(next_cp)) {
                    p = next + next_len;
                    while (p < end) {
                        int c = 0, l = 0;
                        utf8_decode_cp_len(p, end, &c, &l);
                        if (is_space_cp(c) || is_word_cp(c))
                            break;
                        p += l;
                    }
                    while (p < end) {
        ...
        if (append_encoded_piece(tok, (const char *)piece, (size_t)(p - piece), &ids, &n_ids, &cap) !=
            0) {
            free(ids);
            return -1;
        }
...
static int encode_bpe_word(const tok_bpe_t *tok, const char *mapped, int **out_ids, int *out_n) {
    ...
    *out_n = 0;
    ...
    char **syms = NULL;
    int n_syms = 0;
    if (split_utf8_symbols(mapped, &syms, &n_syms) != 0)
        return -1;
    if (n_syms <= 0) {
        free(syms);
        return 0;
...
static int reserve_bpe(tok_bpe_workspace_t *ws, int need) {
    if (ws->bpe_cap >= need)
        return 0;
...
static int reserve_syms(tok_bpe_workspace_t *ws, int need) {
    if (ws->syms_cap >= need)
        return 0;
    ...
    bpe_sym_ref_t *p = (bpe_sym_ref_t *)realloc(ws->syms, (size_t)new_cap * sizeof(*ws->syms));
    if (!p)
        return -1;
    ws->syms = p;
    ws->syms_cap = new_cap;
...
static int arena_append(tok_bpe_workspace_t *ws, const char *s, size_t len, size_t *off) {
    if (ws->arena_len > SIZE_MAX - len - 1)
        return -1;
    size_t need = ws->arena_len + len + 1;
    if (reserve_bytes(&ws->arena, &ws->arena_cap, need) != 0)
        return -1;
    *off = ws->arena_len;
...
static const char *arena_str(const tok_bpe_workspace_t *ws, bpe_sym_ref_t sym) {
    return ws->arena + sym.off;
}

static int normalize_nfc_latin_into(tok_bpe_workspace_t *ws, const char *text, const char **out) {
    const unsigned char *p = (const unsigned char *)text;
    const unsigned char *end = p + strlen(text);
    size_t in_len = (size_t)(end - p);
    if (in_len > (SIZE_MAX - 1) / 2)
        return -1;
    if (reserve_bytes(&ws->normalized, &ws->normalized_cap, in_len * 2 + 1) != 0)
        return -1;
...
static int
text_to_bpe_unicode_into(tok_bpe_workspace_t *ws, const char *text, size_t len, const char **out) {
    init_gpt2_mapping();
    if (len > (SIZE_MAX - 1) / 3)
        return -1;
...
static int output_append_id(int *out_ids, int out_cap, int *n_ids, int *overflow, int id) {
...
static int encode_bpe_word_into_slow(const tok_bpe_t *tok,
    ...
    while (n_syms > 1) {
        ...
        for (int j = 0; j < n_syms - 1; j++) {
            int r = merge_rank(tok, arena_str(ws, ws->syms[j]), arena_str(ws, ws->syms[j + 1]));
            if (r < best_rank) {
                best_rank = r;
                best_i = j;
            }
        ...
        if (la > SIZE_MAX - lb)
            return -1;
        if (reserve_bytes(&ws->arena, &ws->arena_cap, ws->arena_len + la + lb + 1) != 0)
            return -1;
    ...
    for (int j = 0; j < n_syms; j++) {
        int id = map_get((const str_int_entry_t *)tok->vocab_map, tok->vocab_map_cap,
                         arena_str(ws, ws->syms[j]));
        if (id < 0)
            return -1;
...
static int encode_bpe_word_into(const tok_bpe_t *tok,
    ...
    if (!tok->int_merges || !tok->cp_to_id)
        return encode_bpe_word_into_slow(tok, ws, mapped, out_ids, out_cap, n_ids, overflow);
    ...
    for (const unsigned char *p = (const unsigned char *)mapped; *p;) {
        ...
        if (len == 1)
            ...
            cp = 512; /* byte-level alphabet never exceeds two bytes */
    ...
    for (;;) {
        ...
        if (prv[best] != -1)
            rnk[prv[best]] = pair_merge_lookup(tok, ids[prv[best]], ids[best], &mrg[prv[best]]);
        rnk[best] =
            nxt[best] != -1 ? pair_merge_lookup(tok, ids[best], ids[nxt[best]], &mrg[best]) : INT_MAX;
    }
...
static int append_encoded_piece_into(const tok_bpe_t *tok,
                                     tok_bpe_workspace_t *ws,
                                     const char *piece,
                                     size_t piece_len,
                                     int *out_ids,
...
static int encode_pretokenized_into(const tok_bpe_t *tok,
                                    tok_bpe_workspace_t *ws,
                                    const char *text,
                                    int *out_ids,
                                    int out_cap,
    ...
    while (p < end) {
        ...
        if (append_encoded_piece_into(tok, ws, (const char *)piece, (size_t)(p - piece), out_ids,
                                      out_cap, &n_ids, &overflow) != 0)
            return -1;
    }
...
static int derive_merges_path(const char *vocab_path, char *out_path, size_t out_cap) {
    ...
    if (!slash) {
        if (snprintf(out_path, out_cap, "merges.txt") >= (int)out_cap)
            return -1;
        return 0;
    }
    size_t dir_len = (size_t)(slash - vocab_path);
    if (dir_len + 12 >= out_cap)
        return -1;
...
static int load_merges_map(tok_bpe_t *tok, const char *merges_path) {
    ...
    while (fgets(line, sizeof(line), f)) {
        char *a = NULL, *b = NULL;
        if (parse_merge_pair(line, &a, &b) == 0)
            n_pairs++;
    }
    if (n_pairs <= 0) {
        fclose(f);
    ...
    pair_merge_entry_t *pm =
        (pair_merge_entry_t *)calloc((size_t)tok->int_merges_cap, sizeof(pair_merge_entry_t));
    if (pm) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (tok->int_merges) {
            ...
            if (left < 0 || right < 0 || merged < 0) {
                ...
                pair_map_insert((pair_merge_entry_t *)tok->int_merges, tok->int_merges_cap,
                                ((uint64_t)(uint32_t)left << 32) | (uint32_t)right, rank, merged);
            }
        }
        (void)map_insert_owned((str_int_entry_t *)tok->merge_map, tok->merge_map_cap, &key, rank);
        free(key);
        rank++;
    }
    fclose(f);
    ...
    if (tok->int_merges) {
        ...
        if (tok->cp_to_id) {
            ...
            for (int byte = 0; byte < 256 && tok->cp_to_id; byte++) {
                ...
                buf[n] = '\0';
                int id = map_get((const str_int_entry_t *)tok->vocab_map, tok->vocab_map_cap, buf);
                if (id < 0) {
                    free(tok->cp_to_id);
                    tok->cp_to_id = NULL;
                } else {
...
tok_bpe_t *tok_bpe_load(const char *vocab_json_path) {
    ...
    if (!f) {
        fprintf(stderr, "tok_bpe_load: cannot open %s\n", vocab_json_path);
        return NULL;
    }
    ...
    if (derive_merges_path(vocab_json_path, merges_path, sizeof(merges_path)) == 0) {
        if (load_merges_map(tok, merges_path) != 0) {
            fprintf(stderr,
...
tok_bpe_workspace_t *tok_bpe_workspace_new(void) {
    return (tok_bpe_workspace_t *)calloc(1, sizeof(tok_bpe_workspace_t));
}
...
int tok_bpe_encode_into(const tok_bpe_t *tok,
    ...
    int n_ids = 0;
    int rc = encode_pretokenized_into(tok, ws, normalized, out_ids, out_cap, &n_ids);
    if (out_n_tokens)
        *out_n_tokens = n_ids;
    return rc;
}
...
int *tok_bpe_encode_with_workspace(const tok_bpe_t *tok,
    ...
    if (out_n_tokens)
        *out_n_tokens = 0;
    if (!tok || !ws || !text || text[0] == '\0')
        return NULL;

    size_t len = strlen(text);
    size_t cap_sz = len > (SIZE_MAX - 8) / 2 ? SIZE_MAX : len * 2 + 8;
    if (cap_sz > (size_t)INT_MAX)
        return NULL;
    int cap = (int)cap_sz;
    ...
    int n_ids = 0;
    int rc = tok_bpe_encode_into(tok, ws, text, ws->ids, ws->ids_cap, &n_ids);
    if (rc == -2) {
        if (reserve_ints(&ws->ids, &ws->ids_cap, n_ids) != 0)
            return NULL;
        rc = tok_bpe_encode_into(tok, ws, text, ws->ids, ws->ids_cap, &n_ids);
    }
    if (rc != 0 || n_ids <= 0)
        return NULL;
...
void tok_bpe_free(tok_bpe_t *tok) {
    ...
    if (tok->id_to_text) {
        for (int i = 0; i < tok->vocab_size; i++)
            free(tok->id_to_text[i]);
        free(tok->id_to_text);
    }
    if (tok->id_to_bpe) {
        for (int i = 0; i < tok->vocab_size; i++)
            free(tok->id_to_bpe[i]);
        free(tok->id_to_bpe);
    }

    if (tok->merge_map) {
        str_int_entry_t *m = (str_int_entry_t *)tok->merge_map;
        for (int i = 0; i < tok->merge_map_cap; i++)
            free(m[i].key);
        free(tok->merge_map);
    }
    free(tok->vocab_map);
