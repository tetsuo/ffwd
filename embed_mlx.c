...
static int json_string_equals_m(const char *json, const char *key, const char *expected) {
...
static int mlx_parse_config(embed_config_t *cfg, const char *model_dir) {
    ...
    if (!f) {
        fprintf(stderr, "mlx: cannot open %s\n", path);
        return -1;
    }
...
#define GI(k, fb, def)                       \
    do {                                     \
        const char *v = json_find_m(buf, k); \
        cfg->fb = v ? atoi(v) : def;         \
    } while (0)
#define GF(k, fb, def)                       \
    do {                                     \
        const char *v = json_find_m(buf, k); \
        cfg->fb = v ? (float)atof(v) : def;  \
    } while (0)
...
static embed_mlx_ctx_t *mlx_load_range_ex(const char *model_dir,
    ...
    if (mlx_parse_config(&ctx->config, model_dir) != 0) {
        free(ctx);
        return NULL;
    }
